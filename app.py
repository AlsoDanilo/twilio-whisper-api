...
# Importações adicionais para localização e texto
import json

# Importações para PDF e DOCX (se ainda não estiverem lá)
try:
    import PyPDF2
except ImportError:
    PyPDF2 = None
...
# Importações para base64 (se ainda não estiverem lá)
try:
    import base64
except ImportError:
    base64 = None
...
def process_and_send():
    """
    Endpoint unificado para processar e enviar diferentes tipos de mensagens (áudio, imagem, documento, texto, localização) para o ChatWoot.
    
    Body esperado:
    {
        "message_type": "audio" | "image" | "document" | "text" | "location",
        "chatwoot": {
            "api_url": "https://app.chatwoot.com",
            "api_token": "SEU_API_TOKEN",
            "account_id": "SEU_ACCOUNT_ID",
            "inbox_id": "SEU_INBOX_ID", (obrigatório para nova conversa)
            "source_id": "SEU_SOURCE_ID", (obrigatório para nova conversa)
            "conversation_id": "ID_DA_CONVERSA" (opcional, se for conversa existente)
        },
        "twilio_url": "URL_DO_TWILIO", (para audio, image, document)
        "text_content": "CONTEUDO_DO_TEXTO", (para texto)
        "latitude": 0.0, (para localização)
        "longitude": 0.0 (para localização)
    }
    ...
    try:
        data = request.get_json()
        if not data or "message_type" not in data or "chatwoot" not in data:
            return jsonify({"success": False, "error": "Campos 'message_type' e 'chatwoot' são obrigatórios."}), 400

        message_type = data["message_type"]
        chatwoot_config = data["chatwoot"]
        conversation_id = chatwoot_config.get("conversation_id")
        
        content_to_send = None
        file_data_to_send = None
        
        # Processamento baseado no tipo de mensagem
        if message_type == "audio":
            twilio_url = data.get("twilio_url")
            if not twilio_url:
                return jsonify({"success": False, "error": "'twilio_url' é obrigatório para message_type 'audio'."}), 400
            ...
            content_to_send = transcript.text
            # Não há file_data_to_send para áudio, pois enviamos a transcrição como texto

        elif message_type == "image":
            twilio_url = data.get("twilio_url")
            if not twilio_url:
                return jsonify({"success": False, "error": "'twilio_url' é obrigatório para message_type 'image'."}), 400
            ...
            if base64 is None:
                return jsonify({"success": False, "error": "Módulo base64 não disponível."}), 500
            image_base64 = base64.b64encode(image_response.content).decode("utf-8")
            content_type = image_response.headers.get("Content-Type", "image/jpeg")
            ...
            twilio_url = data.get("twilio_url")
            if not twilio_url:
                return jsonify({"success": False, "error": "'twilio_url' é obrigatório para message_type 'document'."}), 400
            ...
            if "pdf" in content_type.lower() or twilio_url.lower().endswith(".pdf"):
                if PyPDF2 is None:
                    return jsonify({"success": False, "error": "Módulo PyPDF2 não disponível para PDFs."}), 500
                pdf_buffer = io.BytesIO(doc_response.content)
                pdf_reader = PyPDF2.PdfReader(pdf_buffer)
                for page in pdf_reader.pages:
                    extracted_text += page.extract_text() + "\n"
                ...
                if docx is None:
                    return jsonify({"success": False, "error": "Módulo docx não disponível para documentos Word."}), 500
                doc_buffer = io.BytesIO(doc_response.content)
                doc = docx.Document(doc_buffer)
                for paragraph in doc.paragraphs:
                    extracted_text += paragraph.text + "\n"
            ...
            content_to_send = extracted_text.strip()
            if should_analyze and content_to_send:
                print("Analisando documento com GPT-4...")
                openai_client = get_openai_client()
                analysis_response = openai_client.chat.completions.create(
                    model="gpt-4.1-mini",
                    messages=[
                        {
                            "role": "user",
                            "content": f"Analise e resuma o seguinte documento:\n\n{content_to_send[:4000]}"
                        }
                    ],
                    max_tokens=1000
                )
                content_to_send = f"Documento analisado: {analysis_response.choices[0].message.content}\n\nConteúdo original:\n{content_to_send}"

        elif message_type == "text":
            text_content = data.get("text_content")
            if not text_content:
                return jsonify({"success": False, "error": "'text_content' é obrigatório para message_type 'text'."}), 400
            ...
            if latitude is None or longitude is None:
                return jsonify({"success": False, "error": "'latitude' e 'longitude' são obrigatórios para message_type 'location'."}), 400
            content_to_send = f"Localização: Latitude {latitude}, Longitude {longitude}"

        else:
            return jsonify({"success": False, "error": f"message_type '{message_type}' não suportado."}), 400

        # Enviar para ChatWoot
        if conversation_id:
            # Conversa existente
            success = send_to_chatwoot_existing_conversation(chatwoot_config, conversation_id, content_to_send, file_data_to_send)
            if not success:
                return jsonify({"success": False, "error": "Falha ao enviar mensagem para conversa existente no ChatWoot."}), 500
        else:
            # Nova conversa
            if "inbox_id" not in chatwoot_config or "source_id" not in chatwoot_config:
                return jsonify({"success": False, "error": "'inbox_id' e 'source_id' são obrigatórios para nova conversa no ChatWoot."}), 400
            
            new_conversation_result = send_to_chatwoot_new_conversation(chatwoot_config, content_to_send, file_data_to_send)
            if not new_conversation_result:
                return jsonify({"success": False, "error": "Falha ao criar e enviar mensagem para nova conversa no ChatWoot."}), 500
            conversation_id = new_conversation_result["conversation_id"]

        return jsonify({"success": True, "message": "Mensagem processada e enviada com sucesso.", "conversation_id": conversation_id}))

    except requests.exceptions.RequestException as e:
        return jsonify({"success": False, "error": f"Erro ao fazer requisição externa: {str(e)}"}), 500
    except Exception as e:
        print(f"Erro no endpoint /process-and-send: {str(e)}")
        return jsonify({"success": False, "error": f"Erro interno: {str(e)}"}), 500

@app.route("/process-text", methods=["POST"])
def process_text():
    """
    Endpoint para receber mensagens de texto e enviá-las para o ChatWoot.
    
    Body esperado:
    {
        "text_content": "CONTEUDO_DO_TEXTO",
        "chatwoot": {
            "api_url": "https://app.chatwoot.com",
            "api_token": "SEU_API_TOKEN",
            "account_id": "SEU_ACCOUNT_ID",
    ...
    try:
        data = request.get_json()
        if not data or "text_content" not in data or "chatwoot" not in data:
            return jsonify({"success": False, "error": "Campos 'text_content' e 'chatwoot' são obrigatórios."}), 400
        ...
        # Enviar para ChatWoot
        if conversation_id:
            success = send_to_chatwoot_existing_conversation(chatwoot_config, conversation_id, text_content)
            if not success:
                return jsonify({"success": False, "error": "Falha ao enviar mensagem de texto para conversa existente no ChatWoot."}), 500
        else:
            if "inbox_id" not in chatwoot_config or "source_id" not in chatwoot_config:
                return jsonify({"success": False, "error": "'inbox_id' e 'source_id' são obrigatórios para nova conversa no ChatWoot."}), 400
            
            new_conversation_result = send_to_chatwoot_new_conversation(chatwoot_config, text_content)
            if not new_conversation_result:
                return jsonify({"success": False, "error": "Falha ao criar e enviar mensagem de texto para nova conversa no ChatWoot."}), 500
            conversation_id = new_conversation_result["conversation_id"]

        return jsonify({"success": True, "message": "Mensagem de texto enviada com sucesso.", "conversation_id": conversation_id})

    except Exception as e:
        print(f"Erro no endpoint /process-text: {str(e)}")
        return jsonify({"success": False, "error": f"Erro interno: {str(e)}"}), 500
...
def process_location():
    """
    Endpoint para receber dados de localização (latitude e longitude), gerar um link do Google Maps e enviá-lo para o ChatWoot.
    ...
    try:
        data = request.get_json()
        if not data or "latitude" not in data or "longitude" not in data or "chatwoot" not in data:
            return jsonify({"success": False, "error": "Campos 'latitude', 'longitude' e 'chatwoot' são obrigatórios."}), 400
        ...
        message_content = f"Localização: {location_link}"

        # Enviar para ChatWoot
        if conversation_id:
            success = send_to_chatwoot_existing_conversation(chatwoot_config, conversation_id, message_content)
            if not success:
                return jsonify({"success": False, "error": "Falha ao enviar localização para conversa existente no ChatWoot."}), 500
        else:
            if "inbox_id" not in chatwoot_config or "source_id" not in chatwoot_config:
                return jsonify({"success": False, "error": "'inbox_id' e 'source_id' são obrigatórios para nova conversa no ChatWoot."}), 400
            
            new_conversation_result = send_to_chatwoot_new_conversation(chatwoot_config, message_content)
            if not new_conversation_result:
                return jsonify({"success": False, "error": "Falha ao criar e enviar localização para nova conversa no ChatWoot."}), 500
            conversation_id = new_conversation_result["conversation_id"]

        return jsonify({"success": True, "message": "Localização enviada com sucesso.", "conversation_id": conversation_id})

    except Exception as e:
        print(f"Erro no endpoint /process-location: {str(e)}")
        return jsonify({"success": False, "error": f"Erro interno: {str(e)}"}), 500
